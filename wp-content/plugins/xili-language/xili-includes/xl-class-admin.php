 ...
 * 2025-03+20 - 2.21.3 - fixes sanitizing (Patchstack)
...
class xili_language_admin extends xili_language {
	...
	function __construct( $xl_parent ){
		...
		//add_action( 'admin_init', array( &$this, 'admin_redirects') ); // 2.20 for welcome screen - not use since 2025
	...
	public function admin_redirects() {
		...
		delete_transient( '_xl_activation_redirect' ); /// to fixes issues
...
// categories
	...
	function admin_enqueue_menu_script() {
		...
		$data = array ();
		$data['ajax_nonce'] = wp_create_nonce('menu_items'); // 20250322
...
<?php
	...
	*/
	...
	/**
	*
	* called by /js/nav-menu.xxx.js when displaying results in nav menu settings - menus selection pour créer l'affichage
	* @since 2.9.20 - modified 250320 - sanitizing
	 *
	*
	* @updated 2.21.3
	*/
	function ajax_get_menu_infos () {
		...
		if ( ! check_ajax_referer( 'menu_items', 'ajax_nonce', false ) ) {  // add to avoid external form !
			wp_nonce_ays();
		} else {
	 ...
	 *
	 * @since 2.9.10 - modified 250320 - sanitizing
	 *
	 */
	function save_bulk_edit_language() {
		...
		$assigned_lang = ( isset( $_POST[ 'assigned_lang' ] ) && ! empty( $_POST[ 'assigned_lang' ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ 'assigned_lang' ])) : NULL;
